...
from flax_addons.utils import audio

test_cases = [
    np.random.rand(i).astype(np.float32) for i in [2048, 10 ** 4, 10 ** 6, 10 ** 8]
]
...
def test_jax_stft(benchmark, x):
    ...
    audio.stft(jnp.zeros(x.shape, dtype=x.dtype))  # warm start
    benchmark(audio.stft, x)
